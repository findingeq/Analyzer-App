...
# ============================================================================
# SIGNAL FILTERING (Hybrid Two-Stage)
# ============================================================================
...
def apply_hybrid_filtering(ve_raw: np.ndarray, breath_times: np.ndarray,
    ...
    Stage 1: Rolling median (breath domain) - removes artifacts
...
def detect_intervals(power_df: pd.DataFrame, breath_df: pd.DataFrame) -> Tuple[RunType, int, float, float]:
    ...
    Uses consensus-based detection with rounding to 1min/0.5min grid.
    Assumes run starts at t=0.
    """
    if power_df.empty or len(power_df) < 10:
        # Default to steady state if no power data
        total_duration = breath_df['breath_time'].max() / 60.0  # Convert to minutes
    ...
    raw_intervals = []  # List of (start_time, end_time) tuples
    raw_rests = []      # List of rest durations
    current_start = 0.0 if in_work else None  # Start at t=0 if in work
    last_work_end = None
    ...
    for i in range(1, len(time)):
        if in_work:
            ...
            if power_smooth[i] < power_threshold - hysteresis:
                ...
                start_t = 0.0 if current_start is None else time[current_start] if isinstance(current_start, int) else current_start
                duration = time[i] - start_t
                if duration >= min_interval_duration:
                    raw_intervals.append((start_t, time[i]))
                    last_work_end = time[i]
                in_work = False
                current_start = None
        else:
            # Check for transition to work
            if power_smooth[i] > power_threshold + hysteresis:
                # Record rest duration if we had a previous work interval
                if last_work_end is not None:
                    rest_duration = time[i] - last_work_end
                    raw_rests.append(rest_duration)
                in_work = True
                current_start = i
    
    # Handle case where we end in a work interval
    if in_work and current_start is not None:
        start_t = time[current_start] if isinstance(current_start, int) else current_start
        duration = time[-1] - start_t
        if duration >= min_interval_duration:
            raw_intervals.append((start_t, time[-1]))
    
    # If no intervals detected or only one, treat as steady state
    if len(raw_intervals) <= 1:
        total_duration = breath_df['breath_time'].max() / 60.0
        return RunType.VT1_STEADY, 1, total_duration, 0.0
    
    # Calculate consensus interval duration (median, then round to nearest 1 minute)
    interval_durations = [(end - start) / 60.0 for start, end in raw_intervals]
    median_interval = np.median(interval_durations)
    rounded_interval = round(median_interval)  # Round to nearest 1 minute
    if rounded_interval < 1:
        rounded_interval = 1
    
    # Calculate consensus rest duration (median, then round to nearest 0.5 minute)
    if raw_rests:
        rest_durations = [r / 60.0 for r in raw_rests]
        median_rest = np.median(rest_durations)
        rounded_rest = round(median_rest * 2) / 2  # Round to nearest 0.5 minute
        if rounded_rest < 0.5:
            rounded_rest = 0.5
    else:
        rounded_rest = 1.0  # Default rest
    
    num_intervals = len(raw_intervals)
    
    return RunType.VT2_INTERVAL, num_intervals, float(rounded_interval), float(rounded_rest)
...
def create_main_chart(breath_df: pd.DataFrame, results: List[IntervalResult],
    ...
    for result in results:
        ...
        # Bold binned VE line (what CUSUM analyzes)
        fig.add_trace(
            go.Scatter(
        ...
        # Expected VE (baseline model) - dashed light green
        fig.add_trace(
            go.Scatter(
                x=result.time_values,
    ...
    if st.session_state.results is not None:
        ...
        with st.expander("ℹ️ How it works"):
            ...
            **1. Data Processing**
            ...
            - Stage 1: Rolling median filter (5 breaths) removes artifacts
            ...
            **2. Interval Detection**
            ...
            - Uses consensus-based detection (median duration across all intervals)
            - Rounds to 1-minute (intervals) and 0.5-minute (rest) increments
            - Assumes run starts at t=0
