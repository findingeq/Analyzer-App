...
def detect_intervals(power_df: pd.DataFrame, breath_df: pd.DataFrame) -> Tuple[RunType, int, float, float]:
    ...
    4. Validate against total duration: total ≈ N × (interval + recovery)
    5. If validation fails, try adjacent recovery durations (±0.5 min) to find best fit
    """
    ...
    is_interval = (p90 > p10 * 1.3 and power_range > 50) or (p90 > p10 * 2)
    ...
    if not is_valid:
        # Try alternative interval durations (±1 min) and recovery durations (±0.5 min)
...
def analyze_interval(breath_df: pd.DataFrame, interval: Interval,
    ...
    # BELOW:      (no alarm + slope ≤ expected) OR (alarm + recovered + slope ≤ expected)
    # BORDERLINE: (no alarm + slope > expected) OR (alarm + recovered + slope > expected) OR (alarm + no recovery + slope ≤ expected)
...
def main():
    ...
    if st.session_state.results is not None:
        ...
        # Show 4 columns if cumulative drift available, otherwise 3
        ...
        # Check if we need cumulative drift column
        is_vt2 = run_type.value == RunType.VT2_INTERVAL.value if hasattr(run_type, 'value') else run_type == RunType.VT2_INTERVAL
        show_cum_drift_col = is_vt2 and cumulative_drift is not None
        ...
        # Header row
        if show_cum_drift_col:
            col_widths = [0.4, 1.4, 1.0, 0.6, 0.6, 0.6, 0.6, 0.9]
            headers = ["#", "Status", "VE Drift", "Peak", "Peak C", "Final C", "Alarm", "Cum Drift"]
        else:
            col_widths = [0.4, 1.4, 1.0, 0.6, 0.6, 0.6, 0.6]
            headers = ["#", "Status", "VE Drift", "Peak", "Peak C", "Final C", "Alarm"]
        ...
        with table_center:
            ...
            for r in results:
                ...
                # Format alarm time
                if r.alarm_time is not None:
                    alarm_rel = r.alarm_time - r.interval.start_time
                    alarm_mins = int(alarm_rel // 60)
                    alarm_secs = int(alarm_rel % 60)
                    alarm_str = f"{alarm_mins}:{alarm_secs:02d}"
                else:
                    alarm_str = "—"
                ...
                with row_container:
                    ...
                    # Peak C as % of threshold
                    peak_c_pct = (r.peak_cusum / r.cusum_threshold * 100) if r.cusum_threshold > 0 else 0
                    row_cols[4].markdown(f"<div style='{cell}'>{peak_c_pct:.1f}%</div>", unsafe_allow_html=True)
                    # Final C as % of threshold, with "(Recovered)" if applicable
                    final_c_pct = (r.final_cusum / r.cusum_threshold * 100) if r.cusum_threshold > 0 else 0
                    final_c_str = f"{final_c_pct:.1f}%"
                    if r.cusum_recovered:
                        final_c_str += " (Rec)"
                    row_cols[5].markdown(f"<div style='{cell}'>{final_c_str}</div>", unsafe_allow_html=True)
                    row_cols[6].markdown(f"<div style='{cell}'>{alarm_str}</div>", unsafe_allow_html=True)
                    if show_cum_drift_col:
                        row_cols[7].markdown(f"<div style='{cell}'>{cum_drift_str}</div>", unsafe_allow_html=True)
